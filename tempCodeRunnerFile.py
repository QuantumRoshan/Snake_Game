...
def game_over():
    ...
    canvas.create_text(canvas.winfo_width() / 2, canvas.winfo_height() / 2 + 100,
                       font=('consolas', 20), text="Score: {}".format(score), fill="white", tag="gameover")

    play_again_button = Button(canvas, text="Play Again", font=('consolas', 20), command=reset_game)
    canvas.create_window(canvas.winfo_width() / 2, canvas.winfo_height() / 2 + 150, window=play_again_button, tag="gameover")
